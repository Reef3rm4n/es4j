...
import io.vertx.skeleton.taskqueue.exceptions.TaskProcessorException;
import io.vertx.skeleton.taskqueue.postgres.PgTransaction;
...
public class TaskProcessorVerticle extends AbstractVerticle {
  ...
  private TransactionManager getTransactionManager() {
    return switch (taskConfiguration.transactionManagerImplementation()) {
      case JOOQ ->
        throw new TaskProcessorException(new Error("transaction manager not supported", "jooq transaction manager is not yet implemented", 999));
  ...
  private TaskSubscriber getSubscriber() {
    return switch (taskConfiguration.queueImplementation()) {
      ...
      case RABBITMQ ->
        throw new TaskProcessorException(new Error("queue type not supported", "rabbit task queue is not yet implemented", 999));
      case SOLACE ->
        throw new TaskProcessorException(new Error("queue type not supported", "solace task queue is not yet implemented", 999));
    };
  }
