...
public record SolrCollection<K, V extends RepositoryRecord<V>, Q extends Query, S, Y extends Query>(
    ...
    return vertx.executeBlocking(Uni.createFrom().item(
          Unchecked.supplier(() -> {
              try {
                ...
                throw new SolrClientException(new EventXError("Solr Collection error " + solrMapper.collection(), e.getMessage(), 500));
              } catch (IOException e) {
                logger.error("IoException, in solr  " + solrMapper.collection(), e);
                throw new SolrClientException(new EventXError("Solr repository error for " + solrMapper.collection(), e.getMessage(), 500));
              }
            }
          )
        )
        .map(Unchecked.function(solrDocument -> {
              if (solrDocument._size() > 1) {
                throw new SolrClientException(new EventXError("Key Query Wrong", "Key query is returning more than one object from solr solrMapper.collection()", 400));
              }
  ...
  public Uni<V> get(K key) {
    return vertx.executeBlocking(Uni.createFrom().item(
      ...
      .map(Unchecked.function(solrDocument -> {
            if (solrDocument._size() > 1) {
              throw new SolrClientException(new EventXError("Key Query Wrong", "Document not found in collection: " + solrMapper.collection(), 400));
            }
            final var solrBean = solrDocument.getBeans(solrMapper.solrObjectClass()).stream()
              .findFirst()
              .orElseThrow(() -> new SolrClientException(new EventXError("Solr Document Not found", "Document not found", 40)));
        ...
        Uni.createFrom().item(
          Unchecked.supplier(() -> {
              try {
                ...
                throw new SolrClientException(new EventXError("Solr Stream interrupted", e.getMessage(), 500));
              } catch (Exception e) {
                logger.error("Exception, error within the callback", e);
                throw new SolrClientException(new EventXError("Callback return error", e.getMessage(), 500));
              }
            }
          )
        )
        ...
        Unchecked.supplier(() -> {
            try {
              ...
              if (queryResult.getResults().isEmpty()) {
                throw new SolrClientException(new EventXError("Not Found", "Empty query result", 400));
              }
              ...
              if (exception instanceof SolrClientException solrException) {
                throw solrException;
              }
              throw new SolrClientException(new EventXError(exception.getMessage(), exception.getLocalizedMessage(), 500));
            }
          }
        )
      )
