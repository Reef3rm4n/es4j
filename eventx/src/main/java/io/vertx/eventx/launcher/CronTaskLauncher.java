...
import io.vertx.eventx.queue.postgres.models.MessageRecordQueryBuilder;
import io.vertx.eventx.sql.models.QueryOptionsBuilder;
...
public class CronTaskLauncher {
  ...
  public static void addProjections(final Injector injector) {
    if (CustomClassLoader.checkPresenceInBinding(injector, CronTask.class)) {
      bootstrap();
      kickstart(injector)
        .subscribe()
        .with(avoid -> LOGGER.info("Cron tasks kick-started")
  ...
  public static Uni<Void> kickstart(Injector injector, List<CronTask> tasks) {
    ...
    return producer.query(query(tasks), CronTaskMessage.class)
      .flatMap(messages -> {
          ...
          final var tasksToKickstart = existingTaskNames.stream()
            ...
            .toList();
          LOGGER.debug("Kick-starting tasks {} ", tasksToKickstart.stream().map(Object::getClass).map(Class::getName).toList());
  ...
  private static MessageRecordQuery query(List<CronTask> tasks) {
    return MessageRecordQueryBuilder.builder()
      .ids(tasks.stream()
        ...
        .toList())
      .options(QueryOptionsBuilder.builder().tenantId("default").build())
      .build();
  }
