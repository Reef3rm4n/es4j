...
import static io.es4j.infrastructure.bus.AddressResolver.commandConsumer;
      ...
      .map(Unchecked.function(
          aBoolean -> {
            if (Boolean.TRUE.equals(aBoolean)) {
              throw new NodeUnavailable(new Es4jError(
                null,
      ...
      .handler(message -> {
        ...
        request(vertx, aggregateClass, command)
          ...
          .with(
            ...
            throwable -> {
              if (throwable instanceof Es4jException es4jException) {
                message.fail(es4jException.error().externalErrorCode(), JsonObject.mapFrom(es4jException.error()).encode());
              } else {
  ...
  private static Throwable transformError(final Throwable throwable) {
    if (throwable instanceof ReplyException reply) {
      if (reply.failureType() == RECIPIENT_FAILURE) {
        try {
          final var error = new JsonObject(reply.getLocalizedMessage()).mapTo(Es4jError.class);
          return new CommandRejected(error);
        } catch (IllegalArgumentException illegalArgument) {
          LOGGER.error("Unable to parse command", illegalArgument);
          return new CommandRejected(new Es4jError(
      ...
      return new CommandRejected(new Es4jError(throwable.getMessage(), null, 500));
    }
  }
