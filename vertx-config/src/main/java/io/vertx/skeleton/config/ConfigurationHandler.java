...
import static io.vertx.skeleton.config.Constants.KUBERNETES;
import static io.vertx.skeleton.config.Constants.KUBERNETES_NAMESPACE;
...
public class ConfigurationHandler {
          ...
          if (CONFIGURATION_FORMAT.equals("yaml")) {
            var stringConfig = configChange.getNewConfiguration().getString(
              configChange.getNewConfiguration()
                ...
                .orElseThrow(GenericVerticleException::illegalState)
            );
  ...
  public static List<TenantBasedConfiguration> extractTenantConfiguration(final JsonObject jsonConfig) {
    ...
    final var tenantConfigurations = mathingKeys.stream().map(key -> {
        final var tenant = Tenant.fromString(key);
        return new TenantBasedConfiguration(tenant, jsonConfig.getJsonObject(key));
