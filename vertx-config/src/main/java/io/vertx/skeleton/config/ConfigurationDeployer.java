          ...
          kubeConfigChange -> {
            final var tenantBasedConfiguration = kubeConfigChange.getMap().entrySet().stream()
              .map(entry -> {
                ...
                final var tenant = Tenant.fromString(entry.getKey());
  ...
  private static Uni<Void> warmCaches(final RepositoryHandler repositoryHandler, final Repository<ConfigurationKey, ConfigurationRecord, ConfigurationQuery> repository) {
    return repository.stream(null,
      configurationRecord -> {
        ...
        final var key = new ConfigurationKey(configurationRecord.name(), configurationRecord.tClass(), configurationRecord.persistedRecord().tenant());
