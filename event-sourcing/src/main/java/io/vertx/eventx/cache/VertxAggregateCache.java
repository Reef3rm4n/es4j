...
public class VertxAggregateCache<T extends Aggregate, V extends EntityState<T>> {
  ...
  public Uni<V> put(AggregateKey k, V v) {
  ...
  public List<Holder<V>> values() {
    return vertx.getDelegate().sharedData().<AggregateKey, Holder<V>>getLocalMap(aggregateClass.getName())
      .values().stream()
      .toList();
  }

  private LocalMap<AggregateKey, Holder<V>> localEntityMap() {
    return vertx.sharedData().<AggregateKey, Holder<V>>getLocalMap(aggregateClass.getName());
  }

  private void removeIfExpired(AggregateKey k) {
  ...
  public V remove(AggregateKey k) {
