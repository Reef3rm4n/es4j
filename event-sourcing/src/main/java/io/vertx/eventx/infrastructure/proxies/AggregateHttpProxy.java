...
public class AggregateHttpProxy implements HttpRoute {
  ...
  private final AggregateEventbusProxy<? extends Aggregate> entityAggregateProxy;
  ...
  public AggregateHttpProxy(
    ...
    this.entityAggregateProxy = new AggregateEventbusProxy<>(vertx,aggregateClass);
  }
  ...
  public void registerRoutes(Router router) {
    ...
    router.get("/" + aggregateClass.getSimpleName().toLowerCase())
      .consumes(Constants.APPLICATION_JSON)
      .produces(Constants.APPLICATION_JSON)
      .handler(routingContext -> entityAggregateProxy.wakeUp(routingContext.body().asJsonObject().mapTo(AggregateRecordKey.class))
        .subscribe()
        .with(
