...
//      .setTcpQuickAck(config.getBoolean("tcpQuickAck", EnvVars.TCP_QUICK_ACK))
  ...
  public Function<Supplier<Uni<RowSet<Row>>>, Uni<Integer>> handleUpdate(Class<?> tClass) {
    ...
    return upstreamSupplier -> upstreamSupplier.get()
      ...
      .failWith(new Conflict(new EventXError(tClass.getSimpleName() + " version mismatch conflict ! unable to updated record", null, 409)))
  ...
  public <T> Function<Supplier<Uni<RowSet<T>>>, Uni<List<T>>> handleUpdateByKeyBatch(Class<T> tClass, int size) {
    ...
    return upstreamSupplier -> upstreamSupplier.get()
      ...
      .map(Unchecked.function(list -> {
            ...
            if (size != list.size()) {
              throw new Conflict(new EventXError("Size mismatch, should be " + size + " but is " + list.size(), "", 500));
            }
            return list;
          }
        )
...
//        Class 22 — Data Exception
        return new DataError(new EventXError(pgException.getErrorMessage(), pgException.getDetail(), 22));
      }
      if (pgException.getCode().startsWith("23")) {
//        Class 23 — Integrity Constraint Violation
        return new IntegrityContraintViolation(new EventXError(pgException.getErrorMessage(), pgException.getDetail(), 23));
...
//        Class 08 — Connection Exception
        return new ConnectionError(new EventXError(pgException.getErrorMessage(), pgException.getDetail(), 50));
      } else {
        return new GenericError(new EventXError(pgException.getErrorMessage(), pgException.getDetail(), 999));
      }
    } else if (throwable instanceof ConnectException connectException) {
      return new ConnectionError(new EventXError("Orm connection refused", connectException.getLocalizedMessage(), 500));
    } else {
      return throwable;
    }
  }
...
//          return row != null && row.iterator().hasNext() ? row.iterator().next().getLong(ID) : null;
  ...
  public <T> Function<Supplier<Uni<RowSet<T>>>, Uni<Void>> handleExists(Class<T> tClass) {
    ...
    return upstreamSupplier -> upstreamSupplier.get()
      ...
      }).onItem().ifNull().failWith(GenericError.duplicated(tClass))
      .onFailure(throwable -> checkError(throwable, tClass))
      .retry().withBackOff(Duration.ofMillis(configuration.getInteger("repositoryRetryBackOff", EnvVars.REPOSITORY_RETRY_BACKOFF))).atMost(configuration.getInteger("repositoryMaxRetry", EnvVars.REPOSITORY_MAX_RETRY))
      .onFailure().transform(this::mapError)
      .replaceWithVoid();
    ...
    return pgPool.getConnection()
      .onFailure().transform(
        Unchecked.function(throwable -> {
          ...
          releaseLock(lock);
          throw new GenericError(new EventXError("Connection timeout", "Row streamer could not obtain connection for type :" + statement, 500));
        })
      )
              ...
              if (lock != null) {
                return sqlConnection.close()
                  ...
                  .call(Unchecked.function((item, throwable1) -> {
                        throw new GenericError(new EventXError("Unable to obtain connection", "Row streamer could not obtain connection for type :" + statement, 500));
                      }
                    )
                  );
              } else {
