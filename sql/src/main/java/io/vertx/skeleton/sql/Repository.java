...
import io.vertx.skeleton.sql.commands.GenerateQueryCommand;
import io.vertx.skeleton.sql.models.Query;
import io.vertx.skeleton.sql.models.RecordRepository;
...
public final class Repository<K extends RepositoryRecordKey, V extends RepositoryRecord<V>, Q extends Query> implements RecordRepository<K, V, Q> {
    ...
    public Uni<V> selectUnique(String statement) {
        ...
        return repositoryHandler.handleSelectUnique(queryGeneratorMapper.actualRecordType).apply(
    ...
    public Uni<V> insert(V value) {
        ...
        return repositoryHandler.handleInsert(value).apply(
            ...
            .map(id -> value.with(value.baseRecord().withId(id)));
    }
    ...
    @Override
    public Uni<Void> stream(Uni<Void> lock, Consumer<V> handler, Q query) {
        ...
        return stream(lock, handler, statement, params);
    }

    @Override
    public Uni<Void> stream(Lock lock, Consumer<V> handler, String statement) {
        logOperation("Streaming", statement);
        return repositoryHandler.handleStreamProcessing(repositoryHandler.pgPool(), lock, statement, queryGeneratorMapper.recordRowMapper, handler)
            .replaceWithVoid();
    }
    ...
    public Uni<Void> stream(Lock lock, Consumer<V> handler, String statement, Tuple arguments, Integer batchSize) {
        logOperation("Streaming", statement, arguments);
        return repositoryHandler.handleStreamProcessing(repositoryHandler.pgPool(), lock, statement, queryGeneratorMapper.recordRowMapper, handler, arguments, batchSize)
            .replaceWithVoid();
    }
    ...
    private static void logOperation(String operation, String statement, Map<String, Object> paramMap) {
        logger.debug(operation + " record with statement -> " + new JsonObject().put("statement", statement).put("params", paramMap).encodePrettily());
    }

    private static void logOperation(String operation, String statement, Tuple paramMap) {
        logger.debug(operation + " record with statement -> " + new JsonObject().put("statement", statement).put("params", paramMap).encodePrettily());
    }

    private static void logOperations(String operation, String statement, List<Map<String, Object>> paramMap) {
        logger.debug(operation + " record with statement -> " + new JsonObject().put("statement", statement).put("params", paramMap).encodePrettily());
    }

    private static void logOperation(String operation, String statement) {
        logger.debug(operation + " record with statement -> " + new JsonObject().put("statement", statement).encodePrettily());
    }

    private static void logOperation(String operation, Object object) {
        logger.debug(operation + " record -> " + JsonObject.mapFrom(object).encodePrettily());
    }

    private static void logOperations(String operation, List<?> object) {
        logger.debug(operation + " records -> " + JsonObject.mapFrom(object).encodePrettily());
    }
