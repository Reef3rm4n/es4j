...
import static io.vertx.skeleton.orm.Constants.CHANGELOG;
import static io.vertx.skeleton.orm.Constants.SCHEMA;
  ...
  public static Uni<Void> runliquibaseChangeLogFile(Vertx vertx, JsonObject config) {
    return UniHelper.toUni(vertx.getDelegate().executeBlocking(
        promise -> {
          ...
          final var changelog = config.getString(CHANGELOG, EnvVars.CHANGELOG);
