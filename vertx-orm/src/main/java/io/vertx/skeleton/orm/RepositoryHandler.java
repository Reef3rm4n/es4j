...
public record RepositoryHandler(
  ...
  private static PoolOptions poolOptions(JsonObject config) {
    return new PoolOptions()
      ...
      .setConnectionTimeout(config.getInteger("pgConnectionTimeOut", io.vertx.skeleton.orm.EnvVars.PG_CONNECTION_TIMEOUT))
      .setPoolCleanerPeriod(config.getInteger("pgPoolCleanerPeriod", io.vertx.skeleton.orm.EnvVars.PG_CLEANER_PERIOD))
//      .setMaxSize(2)
      .setShared(true)
  ...
  private static PoolOptions pooledOptions(JsonObject config) {
    return new PoolOptions()
      ...
      .setConnectionTimeout(config.getInteger("pgConnectionTimeOut", io.vertx.skeleton.orm.EnvVars.PG_CONNECTION_TIMEOUT))
      .setPoolCleanerPeriod(config.getInteger("pgPoolCleanerPeriod", io.vertx.skeleton.orm.EnvVars.PG_CLEANER_PERIOD))
//      .setMaxSize(config.getInteger("pgPoolMaxSize", 4) * CpuCoreSensor.availableProcessors())
      .setShared(true)
  ...
  public static PgConnectOptions connectionOptions(JsonObject config) {
    return new PgConnectOptions()
      ...
      .setLogActivity(config.getBoolean("logActivity", io.vertx.skeleton.orm.EnvVars.LOG_ACTIVITY))
//      .setTcpCork(config.getBoolean("tcpCork", io.vertx.skeleton.orm.EnvVars.TCP_CORK))
//      .setUseAlpn(config.getBoolean("useAlpn", io.vertx.skeleton.orm.EnvVars.USE_ALPN))
//      .setTcpFastOpen(config.getBoolean("tcpFastOpen", io.vertx.skeleton.orm.EnvVars.TCP_FAST_OPEN))
//      .setReusePort(config.getBoolean("reusePort", io.vertx.skeleton.orm.EnvVars.TCP_REUSE_PORT))
//      .setTcpKeepAlive(config.getBoolean("tcpKeepAlive", io.vertx.skeleton.orm.EnvVars.TCP_KEEP_ALIVE))
//      .setTcpNoDelay(config.getBoolean("tcpNoDelay", io.vertx.skeleton.orm.EnvVars.TCP_NO_DELAY))
//      .setTcpQuickAck(config.getBoolean("tcpQuickAck", io.vertx.skeleton.orm.EnvVars.TCP_QUICK_ACK))
      .setReconnectAttempts(config.getInteger("pgReconnect", io.vertx.skeleton.orm.EnvVars.PG_RECONNECT))
      .setSslMode(SslMode.of(config.getString("sslMode", io.vertx.skeleton.orm.EnvVars.SSL_MODE)))
      .setReconnectInterval(config.getInteger("pgReConnectInterval", io.vertx.skeleton.orm.EnvVars.PG_CONNECT_INTERVAL))
      .setHost(config.getString(Constants.PG_HOST, io.vertx.skeleton.orm.EnvVars.PG_HOST))
      .setDatabase(config.getString(Constants.PG_DATABASE, io.vertx.skeleton.orm.EnvVars.PG_DATABASE))
      .setUser(config.getString(Constants.PG_USER, io.vertx.skeleton.orm.EnvVars.PG_USER))
      .setPassword(config.getString(Constants.PG_PASSWORD, io.vertx.skeleton.orm.EnvVars.PG_PASSWORD))
      .setPort(config.getInteger("pgPort", io.vertx.skeleton.orm.EnvVars.PG_PORT))
...
//        Class 22 — Data Exception
        return new OrmDataException(new Error(pgException.getErrorMessage(), pgException.getDetail(), 500));
      }
      if (pgException.getCode().startsWith("23")) {
//        Class 23 — Integrity Constraint Violation
        return new OrmIntegrityContraintViolationException(new Error(pgException.getErrorMessage(), pgException.getDetail(), 409));
...
//        Class 08 — Connection Exception
        return new OrmConnectionException(new Error(pgException.getErrorMessage(), pgException.getDetail(), 500));
      } else {
        return new OrmGenericException(new Error(pgException.getErrorMessage(), pgException.getDetail(), 500));
      }
  ...
  public Function<Supplier<Uni<RowSet<Row>>>, Uni<Long>> handleDelete(Class<?> tClass) {
    ...
    return upstreamSupplier -> upstreamSupplier.get()
      ...
      .failWith(OrmNotFoundException.notFound(tClass.getClass()))
      .onFailure(throwable -> checkError(throwable, tClass))
      .retry().withBackOff(Duration.ofMillis(configuration.getInteger("repositoryRetryBackOff", EnvVars.REPOSITORY_RETRY_BACKOFF))).atMost(configuration.getInteger("repositoryMaxRetry", EnvVars.REPOSITORY_MAX_RETRY))
      .onFailure().transform(this::mapError);
  }
  ...
  private boolean checkError(Throwable throwable, final Class<?> tClass) {
    if (throwable instanceof final PgException pgException) {
      ...
      if (b) {
        logger.debug("Recoverable failure handling statement for" + tClass + " , repository will retry", pgException);
      } else {
    ...
    return pgPool.getConnection()
      .onFailure().transform(
        Unchecked.function(throwable -> {
          ...
          releseLock(lock);
          throw new OrmGenericException(new Error("Connection timeout", "Row streamer could not obtain connection for statement :" + statement, 500));
        })
      )
        ...
        sqlConnection -> sqlConnection.begin()
          ...
          .flatMap(
            transaction -> sqlConnection.prepare(statement)
              ...
              .invoke(aStatement -> logger.debug("Row streamer injected statement"))
              .invoke(
                preparedStatement -> {
  ...
  private void releseLock(final Lock lock) {
    if (lock != null) {
      lock.release();
              ...
              if (lock != null) {
                return sqlConnection.close()
                  ...
                  .call(Unchecked.function((item, throwable1) -> {
                        throw new OrmGenericException(new Error("Unable to obtain connection", "Row streamer could not obtain connection for statement :" + statement, 500));
                      }
                    )
                  );
              } else {
